    ...
    *,
    ...
    with write_txn(conn):
        ...
        conn.execute(
            """
            INSERT OR IGNORE INTO kanban_notify_subs
                (task_id, platform, chat_id, chat_type, thread_id, user_id, user_id_alt,
                 source_profile, notifier_profile, session_key, session_id,
                 message_id, delivery_mode, delivery_metadata, created_at, last_event_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
                    COALESCE((SELECT MAX(id) FROM task_events WHERE task_id = ?), 0))
            """,
            (
                task_id, platform, chat_id, source_chat_type, thread_id or "",
                user_id, user_id_alt, source_profile, owner, canonical_key,
                durable_session_id, str(message_id or ""), insert_mode,
                metadata_json, now,
                task_id,
            ),
        )
        ...
        conn.execute(
            ...
            UPDATE kanban_notify_subs
               SET chat_type = ?, user_id = ?, user_id_alt = ?, source_profile = ?,
                   notifier_profile = ?, session_key = ?, session_id = ?,
                   message_id = ?, delivery_mode = ?
             WHERE task_id = ? AND platform = ? AND chat_id = ? AND thread_id = ?
            """,
            (
                source_chat_type, user_id, user_id_alt, source_profile, owner,
                canonical_key, durable_session_id, str(message_id or ""),
                insert_mode, task_id, platform,
