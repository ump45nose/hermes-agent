...
| `auto_subscribe_on_create` | `true` | When `kanban_create` runs inside a persistent gateway/TUI session, Hermes records the notifier-gateway owner and exact originating session identity. Only that gateway may claim terminal events; it delivers the human notification and resumes the same agent with a synthetic status turn so it can call `kanban_show` and synthesize the handoff. Set to `false` for passive completion or to require explicit subscriptions. Independent of `auto_decompose`. |
...
A chat-originated auto-subscribe is created in `notify+wake` mode: on a terminal event the destination agent both receives the passive message **and** takes a real turn, so it can read the board context and reply in its own voice. An explicit CLI subscription without canonical session identity is notification-only and cannot wake an agent session. See [Delivery modes](#delivery-modes) below.
...
A subscription survives `done` for reopen cycles and removes itself on `archived`; stale `done` subscriptions are bounded by `done_sub_retention_days`. CLI-created subscriptions are notification-only unless a gateway created them with canonical session identity.
