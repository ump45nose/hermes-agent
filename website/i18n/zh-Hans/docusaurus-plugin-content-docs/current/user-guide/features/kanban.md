...
| `auto_subscribe_on_create` | `true` | 当 `kanban_create` 在持久 gateway/TUI 会话中运行时，Hermes 会记录 notifier gateway 所有者和原会话的精确身份。只有所有者 gateway 能领取终态事件；它发送人类通知并通过合成状态回合恢复同一 agent，让编排器调用 `kanban_show` 汇总交接。设为 `false` 可让完成保持被动或要求显式订阅；此设置独立于 `auto_decompose`。 |
...
订阅在任务达到 `done` 后保留，以支持重新打开后的通知；任务进入不可逆的 `archived` 状态时才自动移除。长期不归档的看板会按 `kanban.done_sub_retention_days`（默认 30 天，设为 0 可禁用）清理长期无新事件的 `done` 订阅。如果你用 `--json`（机器输出）脚本化创建，则跳过自动订阅。没有 canonical session identity 的 CLI 显式订阅只能发通知，不能唤醒 agent 会话。
...
订阅在 `done` 后保留以支持重开，在 `archived` 时自动移除；长期无新事件的 `done` 订阅由 `done_sub_retention_days` 限制保留期。
