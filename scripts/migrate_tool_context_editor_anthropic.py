#!/usr/bin/env python3
"""Switch deployed profiles to Anthropic-compatible Tool Context Editor mode."""
...
DEFAULT_PROFILES = ("default", "lingjun", "companion", "ops", "research", "xp")
ANTHROPIC_POLICY = {
    "mode": "anthropic",
...
def migrate(root: Path, profiles: tuple[str, ...]) -> dict:
    ...
    backup = root / "backups" / "tool-context-editor-anthropic" / stamp
    ...
    for profile in profiles:
        config_path = (
            root / "config.yaml"
            if profile == "default"
            else root / "profiles" / profile / "config.yaml"
        )
        ...
        before_policy = {key: editor.get(key) for key in ANTHROPIC_POLICY}
        editor.update(ANTHROPIC_POLICY)
        if before_policy != ANTHROPIC_POLICY:
            _write_yaml_atomic(config_path, config)
        changes.append(
            {"profile": profile, "before": before, "after": "anthropic"}
        )
