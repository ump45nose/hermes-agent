            ...
            for _msg_idx in range(_scan_start, len(messages)):
                ...
                self._session_db.append_message(
                    ...
                    display_kind=(
                        "hidden"
                        if (
                            _durable_msg.get(COMPRESSED_SUMMARY_METADATA_KEY)
                            and (
                                ContextCompressor.classify_summary_content(
                                    _durable_msg.get("content")
                                )
                                == "standalone"
                                or not _durable_msg.get(
                                    "_compressed_summary_has_user_turn"
                                )
                            )
                        )
                        else _durable_msg.get("display_kind")
                    ),
                    display_metadata=_durable_msg.get("display_metadata"),
                ...
                _durable_msg[_DB_PERSISTED_MARKER] = True
