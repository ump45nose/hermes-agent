...
# Variant used when skill tools are not available in the current session.
# Keep the documentation pointer actionable without referring to skill_view().
...
MEMORY_GUIDANCE = (
    "Save profile-private durable facts using the memory tool: user preferences, "
    "environment details, tool quirks, and stable conventions. But shared "
    "environment/runtime facts must use shared_state."
)
...
SKILLS_GUIDANCE = (
    "Skills are curated reusable procedures, not automatic post-task notes. "
    "Do not create or patch a skill for a one-off task, a transient failure, "
    "an environment/runtime fact, a session outcome, or a user/profile fact. "
    "Store profile-private durable facts in memory and shared environment/runtime "
    "facts in shared_state. Create or change a skill only when the user explicitly "
    "asks, or when a stable procedure is clearly reusable across multiple future "
    "tasks and is not already covered by a tool/MCP description or existing skill."
